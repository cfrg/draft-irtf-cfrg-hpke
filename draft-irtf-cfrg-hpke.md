...
* A Key Encapsulation Mechanism (KEM):
  ...
  - DeriveKeyPair(ikm): Derive a key pair `(skX, pkX)` from the byte
    string `ikm`, where `ikm` SHOULD have at least `Nsk` bytes of entropy
    (see {{derive-key-pair}} for discussion)
  - SerializePublicKey(pkX): Produce a byte string of maximum length `Npk`
    encoding the KEM public key `pkX`. A KEM MAY provide multiple
    different equivalent encoding algorithms for public keys, with
    different output lengths.
  - DeserializePublicKey(pkXm): Parse the byte string `pkXm` of length `Npk`
    to recover a KEM public key. This function can raise an error upon
    `pkXm` deserialization failure.
  - SerializeEncapsulatedKey(pkE): Produce a byte string of maximum length `Nenc`
    encoding the KEM ciphertext `pkE` which encapsulates a KEM shared
    secret. A KEM MAY provide multiple different equivalent encoding
    algorithms for KEM ciphertexts, with different output lengths.
  - DeserializeEncapsulatedKey(enc): Parse the byte string `enc` of
  ...
  - Nenc: The maximum length in bytes of an encapsulated key produced by this KEM
...
- Deserialize(enc): Parse a byte string of length `Npk` to recover a
  public key. This function can raise an error upon `enc` deserialization
  failure.
...
algorithm, `SerializePublicKey` and `SerializeEncapsulatedKey` are
equivalent, and `Npk` equals `Nenc`. Thus, we implement both using a
single function called `Serialize`. `Deserialize` is implemented similarly.
...
impersonation. This means the assurance that the KEM shared secret key
...
encoded in the `VerifyPSKInputs` function below.
...
AuthPSK modes, the `psk` and `pskID` arguments are provided as required,
...
to `pkS`.  In other words, only two entities could have produced this
...
* Nenc: The maximum length in bytes of an encoded encapsulated key produced by
  the algorithm.
