...
* A Key Encapsulation Mechanism (KEM):
  ...
  - `SerializePk(pk)`: Produce a byte string of length `Npk` encoding the
    public key `pk`
  - `DeserializePk(enc)`: Parse the byte string `enc` of length `Npk` to recover a
    public key. This function can raise a `DeserializeError` upon
    deserialization failure.
  - `SerializeSk(sk)`: Produce a byte string of length `Nsk` encoding the
    private key `sk`
  - `DeserializeSk(enc)`: Parse the byte string `enc` of length `Nsk` to recover a
    private key. This function can raise a `DeserializeError` upon
    deserialization failure.
  ...
  - `AuthEncap(pkR, skS)` (optional): Same as `Encap()`, and the outputs
    encode an assurance that the KEM shared private key was generated by the
...
- `SerializePk(pk)`: Produce a byte string of length `Npk`
  encoding the public key `pk`
- `DeserializePk(enc)`: Parse a byte string of length `Npk` to recover a
  public key (note: this function can raise an error upon `enc` deserialization failure)
- `SerializeSk(sk)`: Produce a byte string of length `Nsk`
  encoding the private key `sk`
- `DeserializeSk(enc)`: Parse a byte string of length `Nsk` to recover a
  private key (note: this function can raise an error upon `enc` deserialization failure)
...
def Decap(enc, skR):
  pkE = DeserializePk(enc)
  dh = DH(skR, pkE)

  pkRm = SerializePk(pk(skR))
  kem_context = concat(enc, pkRm)
...
impersonation (KCI). This means the assurance that the KEM shared private key
...
### SerializePk/DeserializePk

For P-256, P-384 and P-521, the `SerializePk()` function of the
KEM performs the uncompressed Elliptic-Curve-Point-to-Octet-String
conversion according to {{SECG}}. `DeserializePk()` performs the
uncompressed Octet-String-to-Elliptic-Curve-Point conversion.

For X25519 and X448, the `SerializePk()` and `DeserializePk()` functions
...
For X25519 and X448, the `SerializeSk()` function MUST clamp the private key
before returning its byte string representation. The `DeserializeSk()` function
is the identity function.

Deserialized private keys MUST be validated before they can be used. See
{{validation}} for specifics.
...
used. "Clamping" refers to the bitwise operations performed on `k` in the
